...
- [ ] T041b External pinger to bypass GH scheduler throttling — configure a free external cron (cron-job.org) to POST `workflow_dispatch` to `scraper.yml` every 10 min via the GitHub REST API using a fine-grained PAT scoped to the repo + `actions:write`. `workflow_dispatch` triggers are not subject to the same throttling as `schedule` triggers, so this should restore close-to-144 fires/day. Steps: (1) mint a fine-grained PAT, (2) sign up at cron-job.org, (3) configure POST job per `scraper/pinger.md`, (4) verify with `gh run list -w scraper.yml --event workflow_dispatch --limit 10`. Then re-run T041 over a fresh 24h window.
